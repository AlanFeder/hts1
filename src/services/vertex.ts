...
const PRICE_INPUT_PER_TOKEN = 0.10 / 1_000_000;
const PRICE_OUTPUT_PER_TOKEN = 0.40 / 1_000_000;
...
function makeBatches(texts: string[]): string[][] {
  ...
  for (const text of texts) {
    ...
    if (
      current.length > 0 &&
      (currentChars + n > MAX_CHARS_PER_BATCH || current.length >= MAX_TEXTS_PER_BATCH)
    ) {
...
export async function embedTexts(
  ...
  for (const batch of batches) {
    const response = await client.models.embedContent({
      ...
      contents: batch,
      config: { taskType: taskType as "RETRIEVAL_DOCUMENT" | "RETRIEVAL_QUERY" },
    });
