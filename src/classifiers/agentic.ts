...
import type { ChapterMap, ClassifyResponse, HTSNode } from "../types.js";
...
export class AgenticClassifier {
	...
	async classify(
		...
		console.info(
			`agentic | query=${JSON.stringify(description)} top_k=${topK} beam_width=${bw}`,
		);
		...
		const sortedChapters = [...this.chapters.entries()].sort(([a], [b]) =>
			a.localeCompare(b),
		);
		...
		const chResult = await generateText(
			CHAPTER_PROMPT.replace("{description}", description)
				.replace("{n}", String(bw))
		...
		console.info(
			`agentic | selected chapters: ${JSON.stringify(selectedChCodes)}`,
		);
		beamSteps.push({
		...
		for (let depth = 0; depth < 12; depth++) {
			...
			if (beam.length > MAX_DISPLAY) {
				const { filtered, cost } = await embeddingsPrefilter(
					description,
					beam,
					MAX_DISPLAY,
				);
			...
			const stepResult = await generateText(
				STEP_PROMPT.replace("{description}", description).replace(
					"{options}",
					options,
				),
			);
			...
			const { explore: exploreIndices, finalize: finalizeIndices } =
				parseExploreFinalize(stepResult.text);
		...
		if (finalPool.length > topK) {
			...
			const finalResult = await generateText(
				SELECT_PROMPT.replace("{description}", description)
		...
		console.info(
			`agentic | final results: ${JSON.stringify(finalDescs)} total_cost=$${totalCost.toFixed(6)}`,
		);
		beamSteps.push({
			step: "final_ranking",
