...
import { embedCost, embedQuery, generateText } from "../services/vertex.js";
...
export class RerankClassifier {
	...
	async classify(
		...
		console.info(
			`rerank | query=${JSON.stringify(description)} top_k=${topK} candidate_pool=${pool}`,
		);
		...
		console.info(
			`rerank | retrieved ${candidates.length} candidates from vector store`,
		);

		for (const c of candidates) {
			console.debug(
				`rerank | initial score=${c.score.toFixed(4)} hts=${c.hts_code} desc=${JSON.stringify(c.description)}`,
			);
		}
		...
		const options = candidates
			.map(
				(c, i) =>
					`${i + 1}. [${c.hts_code}] ${c.description} (path: ${c.path.slice(-2).join(" > ")})`,
			)
			.join("\n");

		const rerankResult = await generateText(
			RERANK_PROMPT.replace("{description}", description)
		...
		console.info(
			`rerank | final order: ${reranked.map((r) => r.hts_code).join(", ")}`,
		);
