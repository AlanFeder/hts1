...
import { embedCost, embedQuery, vecNorm } from "../services/vertex.js";
...
export class EmbeddingsClassifier {
	...
	async classify(
		...
		console.info(
			`embeddings | query=${JSON.stringify(description)} top_k=${topK} path_weight=${pathWeight}`,
		);

		const embedding = await embedQuery(description);
		const norm = vecNorm(embedding);
		console.debug(
			`embeddings | query embedding norm=${norm.toFixed(4)} dim=${embedding.length}`,
		);
		...
		for (const r of results) {
			console.info(
				`embeddings | score=${r.score.toFixed(4)} hts=${r.hts_code} desc=${JSON.stringify(r.description)}`,
			);
		}
