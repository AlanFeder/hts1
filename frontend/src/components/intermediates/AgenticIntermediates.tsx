...
export default function AgenticIntermediatesPanel({
  ...
  return (
    <div className="space-y-3">
      <p className="section-label">Beam Search Trace ({data.beam_steps.length} steps)</p>
      <div className="space-y-2">
        {data.beam_steps.map((step, i) => (
          <BeamStepRow key={i} step={step} index={i} />
        ))}
      </div>
    </div>
  );
...
function BeamStepRow({ step, index }: { step: AgenticBeamStep; index: number }) {
