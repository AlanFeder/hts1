...
export default function CompareView() {
	...
	const anyDone = METHODS.some(
		(m) => states[m].status === "success" || states[m].status === "error",
	);
						...
						{METHODS.map((method) => {
							...
							const ms =
								s.status === "success"
									...
									: null;
							const cost =
								s.status === "success" ? (s.data.cost_usd ?? 0) : null;
						...
						<table className="w-full text-sm">
							...
							<tbody className="divide-y divide-slate-100">
								{METHODS.map((method) => {
									const s = states[method];
									const meta = METHOD_META[method];
									const top = s.status === "success" ? s.data.results[0] : null;
										...
										<tr key={method} className="hover:bg-slate-50">
											...
											<td className="py-2.5 pr-4">
												{s.status === "loading" ? (
													...
													<a
														href={`https://hts.usitc.gov/search?query=${top.hts_code}`}
														target="_blank"
					...
					{METHODS.map((method) => {
						...
						return (
							...
							>
								...
								<button
									type="button"
									className={`w-full flex items-center gap-3 px-5 py-3.5 text-left ${meta.bg} ${s.status === "success" ? "cursor-pointer" : "cursor-default"}`}
									onClick={() =>
										s.status === "success"
								...
								>
									<span
										className={`w-3 h-3 rounded-full flex-shrink-0 ${meta.dot}`}
									/>
